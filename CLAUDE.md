...
- **Backend Phase 2 — Nội dung trang & settings vào CMS (phiên 04/07) — build+curl OK, CHƯA push:**
  ...
  - **CMS config:** thêm file **Cài đặt → Site** + collection mới **Trang (Pages)** (5 file) trong `public/admin/config.yml` (label VN, `list` scalar dùng `field:` đơn). YAML đã validate parse OK, serve tại `/admin/config.yml`.
